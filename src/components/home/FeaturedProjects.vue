<template>
  <section class="py-12 md:py-20 bg-gray-50 dark:bg-gray-800 transition-colors duration-300">
    <div class="container mx-auto px-4">
      ...
      >
        <div
          v-for="(project, index) in projectStore.featuredProjects"
          :key="project.id"
          class="project-card bg-white dark:bg-gray-900 rounded-lg shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 cursor-pointer active:scale-95"
          :data-index="index"
      ...
      >
        <RouterLink
          to="/projects"
          class="inline-block px-6 md:px-8 py-3 md:py-4 min-h-[44px] bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-all duration-300 transform hover:scale-105 hover:shadow-lg font-semibold active:scale-95"
        >
          查看更多项目
        </RouterLink>
      </div>
...
onMounted(() => {
  ...
  if (ctaRef.value) {
    ...
    if (ctaButton) {
      fadeInUp(ctaButton, { duration: 0.6, delay: 0.8 })
    }
  }
})
</script>